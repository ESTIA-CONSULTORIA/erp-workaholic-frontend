...
export default function POSPage() {
  ...
  return (
    <AppLayout>
      ...
      {/* Modal cobro — selección de forma de pago + cambio */}
      {showCobro && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.8)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:1000 }}>
          <div style={{ background:'#1e293b', borderRadius:12, padding:24, width:680, border:'1px solid #334155' }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:20 }}>
              {/* Izquierda: selector de métodos */}
              <div>
                <p style={{ fontSize:10, color:'#64748b', margin:'0 0 8px', textTransform:'uppercase', letterSpacing:1, fontWeight:700 }}>
                  Métodos de pago
                </p>
                {pagos.map((pago) => {
                  const mc = METODOS_PAGO.find(m => m.id === pago.method);
                  return (
                    <div key={pago.method} style={{ background:'#0f172a', borderRadius:8, padding:8, marginBottom:6 }}>
                        ...
                        <div style={{ display:'flex', gap:5 }}>
                          {pagos.length > 1 && (
                            <button onClick={() => distribuirResto(pago.method)}
                              style={{ fontSize:9, padding:'1px 6px', borderRadius:4, border:`1px solid ${mc?.color}`, background:'none', color:mc?.color, cursor:'pointer' }}>
                              Resto
                            </button>
                          )}
                          {pagos.length > 1 && (
                      ...
                      <div style={{ display:'flex', gap:6 }}>
                        <input type="number" min="0" step="0.01"
                          style={{ flex:1, padding:'6px 10px', borderRadius:6, border:`1px solid ${mc?.color}44`,
                            background:'#1e293b', color:'#f1f5f9', fontSize:15, fontWeight:700 }}
                          value={pago.amount||''}
                          placeholder={fmt(pagos.length === 1 ? total : 0)}
                          onChange={e => actualizarMonto(pago.method, +e.target.value)}
                          autoFocus={pago.method === 'efectivo'}/>
                        <button onClick={() => actualizarMonto(pago.method, pagos.length === 1 ? total : Math.max(0, total - pagos.filter(p=>p.method!==pago.method).reduce((t,p)=>t+Number(p.amount||0),0)))}
                          style={{ padding:'4px 10px', borderRadius:6, border:`1px solid ${mc?.color}`, background:'none', color:mc?.color, cursor:'pointer', fontSize:11 }}>
                          Exacto
                        </button>
                      </div>
                    </div>
                    ...
                    {METODOS_PAGO.filter(m => !pagos.find(p => p.method === m.id)).map(m => (
                      <button key={m.id} onClick={() => agregarMetodo(m.id)}
                        style={{ padding:'4px 10px', borderRadius:6, fontSize:11, cursor:'pointer',
                          border:`1px solid ${m.color}44`, background:'none', color:`${m.color}bb` }}>
                        + {m.label}
                      </button>
                    ))}
                  </div>
                ...
                {/* Resumen si es mixto */}
                {esMixto && (
                  <div style={{ background:'#0f172a', borderRadius:6, padding:'8px 12px', marginTop:8,
                    display:'flex', justifyContent:'space-between' }}>
                    <span style={{ fontSize:12, color:'#64748b' }}>Total cubierto</span>
                    <span style={{ fontSize:13, fontWeight:700, color: totalPagado >= total ? '#10b981' : '#f59e0b' }}>
              ...
              {/* Derecha: cambio (solo si hay efectivo) */}
              <div>
                {tieneEfectivo && (
                  <>
                    <p style={{ fontSize:10, color:'#64748b', margin:'0 0 8px', textTransform:'uppercase', letterSpacing:1, fontWeight:700 }}>
                    ...
                    <div style={{ background:'#0f172a', borderRadius:8, padding:12, marginBottom:12 }}>
                      ...
                      <input type="number" min={faltaEfectivo} step="10"
                        style={{ width:'100%', padding:'8px 12px', borderRadius:8, border:'1px solid #10b98144',
                          background:'#1e293b', color:'#f1f5f9', fontSize:20, fontWeight:700, textAlign:'right', marginBottom:8 }}
                      ...
                      <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
                        ...
                        <button onClick={() => setConCuanto(faltaEfectivo)}
                          style={{ padding:'4px 10px', borderRadius:6, fontSize:12, border:'1px solid #334155', background:'none', color:'#64748b', cursor:'pointer' }}>
                          Exacto
                        </button>
                      </div>
                    </div>
                    <div style={{ background:'#0f172a', borderRadius:8, padding:16, textAlign:'center' }}>
                      ...
                      <p style={{ fontSize:36, fontWeight:700, margin:0,
                        color: conCuanto >= faltaEfectivo ? '#10b981' : '#475569' }}>
                ...
                {!tieneEfectivo && (
                  <div style={{ display:'flex', alignItems:'center', justifyContent:'center', height:'100%',
                    background:'#0f172a', borderRadius:8, padding:24 }}>
            ...
            <div style={{ display:'flex', gap:8, marginTop:20 }}>
              <button onClick={() => setShowCobro(false)}
                style={{ flex:1, padding:'11px', borderRadius:8, border:'1px solid #334155', background:'none', color:'#64748b', cursor:'pointer', fontSize:13 }}>
                Cancelar
              </button>
              <button onClick={() => {
                if (tieneEfectivo && conCuanto > 0) actualizarMonto('efectivo', conCuanto);
                saleM.mutate();
              }}
                disabled={
                  saleM.isPending ||
                  (esMixto && totalPagado < total) ||
                  (tieneEfectivo && !esMixto && conCuanto < total) ||
                  (tieneEfectivo && esMixto && conCuanto < faltaEfectivo)
                }
                style={{ flex:2, padding:'11px', borderRadius:8, border:'none', fontSize:13, fontWeight:700,
                  background: saleM.isPending ? '#334155'
                    : (esMixto && totalPagado < total) ? '#334155'
                    : (tieneEfectivo && conCuanto < (esMixto ? faltaEfectivo : total)) ? '#334155'
                    : color,
                  color:'#fff', cursor:'pointer' }}>
                {saleM.isPending ? 'Procesando…'
                  : tieneEfectivo ? `Confirmar — Cambio ${fmt(Math.max(0, conCuanto - (esMixto ? faltaEfectivo : total)))}`
                  : `Confirmar — ${fmt(total)}`}
              </button>
            </div>
          </div>
